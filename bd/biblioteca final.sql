...
-- Tiempo de generación: 09-03-2021 a las 12:15:55
...
(1, 'Manslaughter', 'Dewain Long', 'Aliquam sit amet diam in magna bibendum imperdiet. Nullam orci pede, venenatis non, sodales sed, tincidunt eu, felis. Fusce posuere felis sed lacus. Morbi sem mauris, laoreet ut, rhoncus aliquet, pulvinar sed, nisl. Nunc rhoncus dui vel sem. Sed sagittis.', 2, 'Drama', 'https://picsum.photos/200/250', 64),
...
(10, 'Once Upon a Forest', 'Cate Hodcroft', 'Aenean fermentum. Donec ut mauris eget massa tempor convallis. Nulla neque libero, convallis eget, eleifend luctus, ultricies eu, nibh. Quisque id justo sit amet sapien dignissim vestibulum. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Nulla dapibus dolor vel est. Donec odio justo, sollicitudin ut, suscipit a, feugiat et, eros. Vestibulum ac est lacinia nisi venenatis tristique. Fusce congue, diam id ornare imperdiet, sapien urna pretium nisl, ut volutpat sapien arcu sed augue. Aliquam erat volutpat.', 8, 'Adventure|Animation|Children|Fantasy', 'https://picsum.photos/200/250', 64),
...
(13, 'El Hechicero', 'David Jiménez Villarejo', 'Un joven mago intenta descubrir quién es, por lo que el destino le lleva a la ciudad Reindam.', 10, 'Accion|Drama', 'https://picsum.photos/200/250', 0);
...
--
ALTER TABLE `Puntuaciones`
  MODIFY `idPuntuacion` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;

--
-- AUTO_INCREMENT de la tabla `Usuarios`
--
ALTER TABLE `Usuarios`
  MODIFY `idUsuario` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;
...
--
ALTER TABLE `Libros_prestados`
  ADD CONSTRAINT `fk_Libros_prestados_Libros` FOREIGN KEY (`idLibro`) REFERENCES `Libros` (`idLibro`) ON DELETE CASCADE ON UPDATE RESTRICT,
  ADD CONSTRAINT `fk_Libros_prestados_Usuarios` FOREIGN KEY (`idUsuario`) REFERENCES `Usuarios` (`idUsuario`) ON DELETE CASCADE ON UPDATE RESTRICT;

--
-- Filtros para la tabla `Lista_deseados`
--
ALTER TABLE `Lista_deseados`
  ADD CONSTRAINT `fk_Usuarios_has_Libros_Libros1` FOREIGN KEY (`idLibro`) REFERENCES `Libros` (`idLibro`) ON DELETE CASCADE ON UPDATE RESTRICT,
  ADD CONSTRAINT `fk_Usuarios_has_Libros_Usuarios2` FOREIGN KEY (`idUsuario`) REFERENCES `Usuarios` (`idUsuario`) ON DELETE CASCADE ON UPDATE RESTRICT;
